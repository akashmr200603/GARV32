...
module rv5_gpio #(
  ...
  reg [GPIO_WIDTH-1:0] gpio_input_sync1;
  reg [GPIO_WIDTH-1:0] gpio_input_sync2;

  always @(posedge clock) begin
    if (reset) begin
      oe <= {GPIO_WIDTH{1'b0}};
      out <= {GPIO_WIDTH{1'b0}};
      gpio_input_sync1 <= {GPIO_WIDTH{1'b0}};
      gpio_input_sync2 <= {GPIO_WIDTH{1'b0}};
    end else begin
      gpio_input_sync1 <= gpio_input;
      gpio_input_sync2 <= gpio_input_sync1;
  ...
  always @(posedge clock) begin
    if (reset) begin
      ...
      if (read_request && address_aligned) begin
        case (address)
          REG_IN    : read_data <= {{32-GPIO_WIDTH{1'b0}}, gpio_input_sync2};
