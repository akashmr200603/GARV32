...
module rv5_spi #(
  ...
  // Rising-edge detect for clk_edge, keeping rx_reg in the system clock domain.
  // This avoids a gated clock and eliminates TIMING-17 critical warnings.
  reg clk_edge_d;
  wire clk_edge_rise;
  always @(posedge clock) begin
    clk_edge_d <= clk_edge;
  end
  assign clk_edge_rise = clk_edge & ~clk_edge_d;

  always @(posedge clock) begin
    if (reset)
      rx_reg <= 8'h00;
    else if (clk_edge_rise)
